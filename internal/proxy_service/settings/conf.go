...
// 读取基本配置
func initProxyConf(path string) (err error) {
	err = ParseConfig(path, "proxy", ConfProxy)
	if err != nil {
		return err
	}

	if ConfProxy.Mode == "" {
		ConfProxy.Mode = "debug"
	}

	if ConfProxy.TimeLocation == "" {
		ConfProxy.TimeLocation = "Asia/Shanghai"
	}
